        ...
        if request.method=="POST":
            ...
            if msg == "clear <jeremy_data>":
                data.child(myid).child("jeremy").cremove()
                 ...
                 response_ids = model.generate(input_ids, max_length=max_length, num_return_sequences=1,pad_token_id=tokenizer.eos_token_id, temperature=temperature, top_k=top_k,do_sample=True)
                 response = tokenizer.decode(response_ids[0], skip_special_tokens=True)
                 resp= response.lstrip(msg).strip()
                 userdat = "✨ Jeremy"
...
if __name__=="__main__":
    app.run(debug=True,port=8000)#may change ;)
